...
$dottedPsw = str_repeat('*', strlen($new_password));
...
if($conn->connect_error){
    ...
    $stmt = $conn->prepare("UPDATE login_form SET new_password = ? WHERE ID = ?");
    $stmt = $conn->prepare("insert into login_form(ID, user_name, new_password)
    values(?, ?, ?)");
    $stmt->bind_param("sss", $ID, $user_name, $dottedPsw);
    $stmt->execute();
    echo "<div style='text-align: center; font-size:30px;margin-top:100px'><p>Password Updated Successfully.</p></div>";
    $stmt->close();
    $conn->close();
}
...
<h2 style="text-align: center; margin-top:100px;">Access to the Portfolio Form <a href='Feedback.html'><br>Go to the Portfolio Form.</br></a></h2>
</body>
</html>